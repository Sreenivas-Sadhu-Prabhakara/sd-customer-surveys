...
| **Asset Type** | Customer Survey |
| **Control Record** | Customer Survey Operating Session |
...
| POST | `/v1/customer-survey-operating-session/initiate` | Initiate |
| GET | `/v1/customer-survey-operating-session` | Retrieve (list) |
| GET | `/v1/customer-survey-operating-session/{crId}/retrieve` | Retrieve |
| PUT | `/v1/customer-survey-operating-session/{crId}/update` | Update |
| PUT | `/v1/customer-survey-operating-session/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/customer-survey-operating-session/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-customer-surveys:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-customer-surveys ./helm -n bian-sales-service
```

Exposed through the platform Gateway at path prefix `/sd-customer-surveys` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
